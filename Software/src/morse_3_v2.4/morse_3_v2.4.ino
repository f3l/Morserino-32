...
//////// HARDWARE definitions that have not been changed between board version 2 and 3 (ESP32 WIFI LORA V.1 and V.2)
...
static unsigned char wifi_bits[] = { 0x00, 0x08, 0x10, 0x24, 0x29, 0x2b, 0x29, 0x24, 0x10, 0x08, 0x00 };
...
void loop()
{
    ...
    case morseGenerator:
        if (p_autoStop)
        {
            if ((autoStop == stop1) || leftKey || rightKey)
    ...
    case 1:
        ...
        break;
    case 2:
        setupPreferences(p_menuPtr); // double click shows the preferences menu (true would select a specific option only)
...
void menu_()
{
    ...
    */
    ...
    { // we wait for a click (= selection)
        ...
        { // actions based on enocder button
        case 2:
            if (setupPreferences(newMenuPtr)) // all available options when called from top menu
        ...
        {
        case -1:
            audioLevelAdjust(); /// for adjusting line-in audio level (at the same time keying tx and sending oudio on line-out
            display.clear();
            menuDisplay(disp);
            break;
...
boolean doPaddleIambic(boolean dit, boolean dah)
{
    ...
    case DIT:
        ...
        {
        case IAMBICB:
            curtistimer = 2 + (ditLength * p_curtisBDotTiming / 100);
    ...
    case DAH:
        ...
        {
        case IAMBICB:
            curtistimer = 2 + (dahLength * p_curtisBTiming / 100); // enhanced Curtis mode B starts checking after some time
            break;
        case NONSQUEEZE:
            curtistimer = 3;
            break;
...
void dispWifiLogo()
{ // display a small logo in the top right corner to indicate we operate with WiFi
    display.setColor(BLACK);
            ...
            if (clearText != "+")
            {
                ...
                case KOCH_MIXED:
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        clearText = getRandomChars(p_randomLength, OPT_KOCH); // Koch option!
                        break;
                    }
                    break;
                case PLAYER:
...
{
    printOnScroll(2, REGULAR, 1, p_autoStop ? "ON         " : "OFF        ");
}
...
{ // this function creates an array of words that are compliant to Koch filter and max word length
    numberOfAbbr = 0;
    for (int i = ABBREV_POINTER[maxl]; i < ABBREV_NUMBER_OF_ELEMENTS; ++i)
    { // do this for all words with max length maxl
...
{ /// measure battery voltage and return result sin milliVolts
...
void cwForTx(int element)
{
    ...
    else if (pairCounter > sizeof(cwTxBuffer) * 4 - 1)
...
void uploadFile()
{
    ...
    server.on(
        "/update", HTTP_POST, // if the client posts to the upload page
        []() { server.sendHeader("Connection", "close");
